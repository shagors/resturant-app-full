        ...
        <div className='hidden md:flex w-full h-full items-center justify-between'>
          ...
          <div className='flex items-center gap-8'>
            ...
            <div className='relative'>
              ...
              {
                isMenu && (
                <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.6 }} className='flex flex-col w-40 bg-gray-50 shadow-xl rounded-lg absolute top-12 right-0'>
                    {
                      user && user.email === "minatouzumaki7400@gmail.com" && (
                        <Link to={"/createItem"}>
            ...
            <motion.img whileTap={{ scale: 0.6 }} src={user ? user.photoURL : Avatar} alt="userprofile" className='w-10 min-w-[40px] h-10 min-h-[40px] shadow-2xl drop-shadow-xl cursor-pointer rounded-full' onClick={login} />
            {
              isMenu && (
                <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.6 }} className='flex flex-col w-40 bg-gray-50 shadow-xl rounded-lg absolute top-12 right-0'>
                  ...
                  <p className="m-2 p-2 flex items-center justify-center bg-gray-200 gap-3 cursor-pointer hover:bg-gray-300 transition-all duration-100 ease-in-out text-textColor text-base rounded-md shadow-md" onClick={logout}>Logout <MdLogout /></p>
                </motion.div>
              )
            }
          </div>
