        ...
        <div className='w-full flex flex-col items-center justify-center'>
            ...
            <div className='w-full flex items-center justify-start lg:justify-center gap-8 py-6 overflow-x-scroll scrollbar-none'>
                {categories && categories.map(category => (
                    <motion.div whileTap={{scale : 0.75}} key={category?.id} className={`group ${filter === category.urlParamName ? 'bg-cartNumBg' : 'bg-card'} w-24 min-w-[94px] h-28 cursor-pointer rounded-lg drop-shadow-xl flex flex-col gap-3 items-center justify-center hover:bg-cartNumBg`} onClick={() => setFilter(category.urlParamName)}>
                        <div className={`w-10 h-10 rounded-full ${filter === category.urlParamName ? 'bg-white' : 'bg-cartNumBg'} group-hover:bg-white shadow-lg flex items-center justify-center`}>
            ...
            <div className='w-full'>
                ...
                data={foodItems?.filter((n) => n.category === filter)}></RowContainer>
            </div>
        </div>
    </section>
  )
